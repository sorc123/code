    ...
    public partial class Form5 : Form
    {
        ...
        public void ucitajDVG()
        {
            try
            {
                SqlConnection con = new SqlConnection(@"Data Source=Teodor-PC\SQLEXPRESS;Initial Catalog=B7_8;Integrated Security=True");
                con.Open();
                SqlDataAdapter com = new SqlDataAdapter("select   Hotel.Naziv,Count(turisticki_aranzman.HotelID)/2 as BrojRezervacija from Hotel join Rezervacija_hotela on Rezervacija_hotela.HotelID = Hotel.HotelID join Turisticki_Aranzman on Turisticki_Aranzman.HotelID = Hotel.HotelID join Soba on Soba.SobaID = Rezervacija_hotela.SobaID join Tip_sobe on Tip_sobe.TipSobeID = Soba.TipSobeID join Turisticka_destinacija on Turisticka_destinacija.DestinacijaID = Turisticki_Aranzman.DestinacijaID where Tip_sobe.Tip = @p1 and Turisticka_destinacija.Drzava = @p2 and Rezervacija_hotela.DatumRezevacije between @p3 and @p4 group by Hotel.Naziv order by Hotel.Naziv asc", con);
